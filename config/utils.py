...
def metric(dataset_preds: np.array, dataset_target: np.array):
    pred = dataset_preds
    ...
    metr_dict = {'F1' : F1, 'MCC': MCC, 'TPR': TPR, 'TNR': TNR, 'PPV': PPV, 'NPV': NPV, 'FPR': FPR, 'FNR': FNR,
                 'BAC': BAC, 'AUC': auc}
    return s, metr_dict  # f'TP {TP:.2f} ,TN {TN:.2f} FP {FP:.2f},FN {FN:.2f},\n F1 {F1 :.4f}   MCC {MCC :.4f}\n TPR
    # {TPR:.4f} TNR ' \

    # f'{TNR:.4f}  PPV {PPV:.4f}\nNPV {NPV:.4f} FPR {FPR:.4f} FNR {FNR:.4f} BAC {BAC:.4f}'


def protein_metric(dataset_preds, dataset_target):
    BATCH_SIZE = dataset_preds.shape[0]
    ...
    for i in range(BATCH_SIZE):
        ...
        confusion_matrix = sklearn.metrics.confusion_matrix(target_, prediction_)
        # print(confusion_matrix)
        cm = confusion_matrix.ravel()
        ...
        MCC = (PPV * TPR * TNR * NPV) ** 0.5 - ((FDR * FNR * FPR * FOR) ** 0.5)
        # print(FPR,TPR)
        fpr, tpr, thresholds = sklearn.metrics.roc_curve(target_, prediction_)
        auc = sklearn.metrics.auc(fpr, tpr)
        s = f'\nTP\tTN\tFP\tFN\t\n{TP:.2f}\t{TN:.2f}\t{FP:.2f}\t{FN:.2f}\n' \
            f'F1  \t  MCC  \t  TPR  \t  TNR  \t  PPV  \t  NPV  \t  FPR  \t  FNR  \t  BAC\t AUC\t\n' \
            f'{F1 :.4f}\t{MCC :.4f}\t{TPR:.4f}\t{TNR:.4f}\t{PPV:.4f}\t{NPV:.4f}\t{FPR:.4f}\t{FNR:.4f}\t{BAC:.4f}\t' \
            f'{auc:.4f}'
        metr_dict = {'F1': F1, 'MCC': MCC, TPR: TPR, }
